...
int main(){
    ...
    if(validUsername(username)){
        ...
        std::string certPath = "/home/"+PATH_USER+"/in-secure_ring/ca/certs/"+username+".crt";
        std::string keyPath = "";
        ...
        const std::string command = "echo -n " + message + " | openssl dgst -sha256 -sign " + keyPath + " -out /home/"+PATH_USER+"/in-secure_ring/ca/private/firma.sha256";
        //std::cout << command <<std::endl;
        const std::string fullCommand = "bash -c \"" + command + "\"";
        std::system(fullCommand.c_str());

        signature = fileManager.Read("/home/"+PATH_USER+"/in-secure_ring/ca/private/firma.sha256");
