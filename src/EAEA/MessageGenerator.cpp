...
int main(){
    ...
    std::string certPath = "/home/mariana.murilloquintana/in-secure_ring/src/EAEA/ca/certs/"+username+".crt";
    std::string keyPath = "";
    ...
    // std::string extractPubKey = "openssl x509 -pubkey -noout -in "+certPath+" > /home/mariana.murilloquintana/in-secure_ring/src/EAEA/ca/private/pubkey.pem";
    // std::system(extractPubKey.c_str());
    // std::string verifyCommand = "bash -c \"echo -n '" + message + "' | openssl dgst -sha256 -verify <(openssl rsa -pubin -inform PEM -in /home/mariana.murilloquintana/in-secure_ring/src/EAEA/ca/private/pubkey.pem) -signature /home/mariana.murilloquintana/in-secure_ring/src/EAEA/ca/private/firma.sha256\"";
